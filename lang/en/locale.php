<?php

return [
    "Apps & Pages" => "Form Modülü",
    "User Interface" => "Personel İşlemleri",
    "Dashboards" => "Ana Sayfa",
    "eCommerce" => "Genel Bilgiler",
    "Apps" => "Anwendungen",
    "Forms & Tables" => "Takımlar ve Projeler",
    "Pages" => "Seiten",
    "Charts & Maps" => "Yönetim Modülü",
    "Others" => "Andere",
    "Email" => "Başvurular",
    "Chat" => "Formlar",
    "Todo" => "Machen",
    "Calendar" => "Takvim",
    "Ecommerce" => "E-Commerce",
    "Shop" => "Geschäft",
    "Wishlist" => "Wunschzettel",
    "Checkout" => "Auschecken",
    "Starter kit" => "Starter-Kit",
    "1 column" => "1 Spalte",
    "2 columns" => "2 Spalten",
    "Fixed navbar" => "Feste Navigationsleiste",
    "Floating navbar" => "Schwimmende Navigationsleiste",
    "Fixed layout" => "Festes Layout",
    "Static layout" => "Statisches Layout",
    "Dark layout" => "Dunkles Layout",
    "Light layout" => "Helles Layout",
    "Data List" => "Datenliste",
    "List View" => "Listenansicht",
    "Thumb View" => "Daumenansicht",
    "Content" => "Inhalt",
    "Grid" => "Gitter",
    "Typography" => "Personeller",
    "Text Utilities" => "Textdienstprogramme",
    "Syntax Highlighter" => "Syntax Textmarker",
    "Helper Classes" => "Hilfsklassen",
    "Colors" => "Farben",
    "Icons" => "Icons",
    "Feather" => "Personel Ödemeleri",
    "Font Awesome" => "Guss Ehrfürchtig",
    "Card" => "Karte",
    "Basic" => "Basic",
    "Advance" => "Voraus",
    "Advanced" => "Voraus",
    "Statistics" => "Statistiken",
    "Card Actions" => "Kartenaktionen",
    "Table" => "Personel İzinleri",
    "Datatable" => "Datentabelle",
    "Components" => "Komponenten",
    "Alerts" => "Warnungen",
    "Buttons" => "Tasten",
    "Breadcrumbs" => "Semmelbrösel",
    "Carousel" => "Karussell",
    "Collapse" => "Zusammenbruch",
    "Dropdowns" => "Dropdowns",
    "List Group" => "Listengruppe",
    "Modals" => "Modals",
    "Modal Examples" => "Modale Beispiele",
    "Pagination" => "Seitennummerierung",
    "Navs Component" => "Navs-Komponente",
    "Navbar" => "Navbar",
    "Tabs Component" => "Registerkarten-Komponente",
    "Pills Component" => "Pillenkomponente",
    "Tooltips" => "Tooltips",
    "Popovers" => "Popovers",
    "Badges" => "Abzeichen",
    "Pill Badges" => "Pillenabzeichen",
    "Progress" => "Fortschritt",
    "Spinner" => "Spinner",
    "Toasts" => "Toast",
    "Extra Components" => "Zusätzliche Komponenten",
    "Avatar" => "Benutzerbild",
    "Chips" => "Chips",
    "Divider" => "Teiler",
    "Form Elements" => "Formularelemente",
    "Select" => "Wählen",
    "Switch" => "Schalter",
    "Checkbox" => "Kontrollkästchen",
    "Radio" => "Radio",
    "Input" => "Eingang",
    "Input Groups" => "Eingabegruppen",
    "Number Input" => "Zahleneingabe",
    "Textarea" => "Textarea",
    "Date & Time Picker" => "Datums- und Uhrzeitauswahl",
    "Form Layout" => "Takımlar",
    "Form Wizard" => "Projeler",
    "Form Validation" => "Müşteriler",
    "Authentication" => "Authentifizierung",
    "Login" => "Anmeldung",
    "Login" => "Anmeldung",
    "Register" => "Registrieren",
    "Register" => "Registrieren",
    "Forgot Password" => "Passwort vergessen",
    "Forgot Password" => "Passwort vergessen",
    "Reset Password" => "Passwort zurücksetzen",
    "Reset Password" => "Passwort zurücksetzen",
    "Coming Soon" => "Demnächst",
    "Error" => "Error",
    "404" => "404",
    "500" => "500",
    "Not Authorized" => "Nicht berechtigt",
    "Maintenance" => "Instandhaltung",
    "Profile" => "Profil",
    "FAQ" => "FAQ",
    "Knowledge Base" => "Wissensbasis",
    "Search" => "Suche",
    "Invoice" => "Rechnung",
    "Charts" => "Diagramme",
    "Apex" => "Apex",
    "Chartjs" => "Chartjs",
    "Echarts" => "Echarts",
    "Google Maps" => "Google Maps",
    "Sweet Alert" => "Süßer Alarm",
    "Toastr" => "Toastr",
    "Sliders" => "Slider",
    "File Uploader" => "Datei-Uploader",
    "Quill Editor" => "Quill Editor",
    "Drag & Drop" => "Ziehen und loslassen",
    "Tour" => "Tour",
    "Clipboard" => "Zwischenablage",
    "Context Menu" => "Kontextmenü",
    "l18n" => "18n",
    "Menu Levels" => "Menüebenen",
    "Second Level 2.1" => "zweites Level 2.1",
    "Second Level 2.2" => "zweites Level 2.2",
    "Third Level 3.1" => "Drittes Level 3.1",
    "Third Level 3.2" => "Drittes Level 3.2",
    "Disabled Menu" => "Deaktiviertes Menü",
    "Documentation" => "Dokumentation",
    "Raise Support" => "Unterstützung erhöhen",
    "Miscellaneous" => "Sonstiges",
    "Extensions" => "Erweiterungen",
    "Media Player" => "Media Player",
    "User Settings" => "Benutzereinstellungen",
    "User" => "Benutzer",
    "List" => "Liste",
    "View" => "Ansehen",
    "Edit" => "Bearbeiten",
    "Account Settings" => "Kontoeinstellungen",
    "Error 404" => "Error 404",
    "Error 405" => "Error 405",
    "Details" => "Einzelheiten",
    "Swiper" => "Swiper",
    "I18n" => "I18n",
    "Access Control" => "Zugangskontrolle",
    "File Manager" => "DateimanagerFile ManagerFile Manager",
    "Pricing" => "Preisgestaltung",
    "Kanban" => "Kanban",
    "Preview" => "Vorschau",
    "Add" => "Hinzufügen",
    "Blog" => "Blog",
    "Detail" => "Detail",
    "Mail Template" => "Mail-Vorlage",
    "Welcome" => "Herzlich willkommen",
    "Forgot" => "Vergessen",
    "Verify" => "Überprüfen",
    "Deactivate" => "Deaktivieren",
    "Order" => "Auftrag",
    "Page Layouts" => "Seitenlayouts",
    "Collapsed Menu" => "Menü reduzieren",
    "Layout Boxed" => "Layout Full",
    "Without Menu" => "Ohne Menü",
    "Layout Empty" => "Layout leer",
    "Layout Blank" => "Layout leer",
    "Form Repeater" => "Form Repeater",
    "Leaflet Maps" => "Kullanıcılar",
    "Misc" => "Sonstiges",
    "Input Mask" => "Eingabemaske",
    "Timeline" => "Zeitleiste",
    "BlockUI" => "BlockUI",
    "Tree" => "Baum",
    "Ratings" => "Bewertungen",
    "Locale" => "Gebietsschema",
    "Reset Password" => "Passwort zurücksetzen",
    "Verify Email" => "E-Mail bestätigen",
    "Deactivate Account" => "Benutzerkonto deaktivieren",
    "Promotional" => "Werbung",
    'Accordion' => 'Akkordeon',
    "Offcanvas" => "Off-Canvas",
    "Custom Options" => "Benutzerdefinierte Optionen",
    "License" => "Lizenz",
    "API Key" => "API-Schlüssel",
    "Two Steps" => "Zwei schritte",
    "Verify Email" => "E-Mail bestätigen",
    "Multi-Steps" => "Mehrere Schritte",
    "Account" => "Konto",
    "Security" => "Sicherheit",
    "Billings & Plans" => "Abrechnungen und Pläne",
    "Notifications" => "Benachrichtigungen",
    "Connections" => "Anschlüsse",
    "Roles & Permission" => "Rollen & Berechtigungen",
    "Roles" => "Rollen",
    "Permission" => "Genehmigung",
    "Cover" => "Abdeckung",
    "message" => "Kuchen Sesam Snaps Cupcake Lebkuchen dänisch Ich liebe Lebkuchen. Apfelkuchen Jujubes Chupa Chups Muffin Halwa Lutscher. Schokoladenkuchen-Haferkuchen-Tiramisumarzipanzuckerpflaume. Donut süße Torte Hafer Kuchen Dragée Obstkuchen Zuckerwatte Zitronentropfen."
];
